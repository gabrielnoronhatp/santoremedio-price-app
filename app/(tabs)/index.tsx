...
export default function HomeScreen() {
  ...
  const [descriptionMap, setDescriptionMap] = useState<Map<string, any>>(
    new Map()
  );
    ...
    return memoize((searchValue: string) => {
      ...
      switch (searchParameter) {
        case "descricao":
          ...
          const matchingKeys = Array.from(descriptionMap.keys()).filter((key) =>
            searchWords.every((word) => key.includes(word))
          );
          return matchingKeys.length > 0
            ? descriptionMap.get(matchingKeys[0])
            : null;
  ...
  const handleConfirm = () => {
    ...
    const newItem = {
      ...
      ean: product ? product.codigoean : searchValue,
      price: formatCurrency(price),
      productName: product ? product.descricao : "Produto não encontrado",
      brand: product ? product.marca : "Marca desconhecida",
      location: location,
    };
